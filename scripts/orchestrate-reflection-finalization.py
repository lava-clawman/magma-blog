...
def main() -> int:
    ...
    try:
        ...
        git_add = run(
            ['git', 'add'] + [str(f) for f in candidate_files if Path(f).exists()],
        ...
        if not success_flag.exists():
            notify(f'magma-blog 自动发布已完成（{date}）\n- 状态：正式稿已发布\n- 流程：Claude Code 草稿 → worker-general 终稿生成 → 脚本发布')
