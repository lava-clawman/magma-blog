...
- [[Youthline]] WFH 邮件处理闭环：先误用 Apple Mail 创建草稿，随后按 Shaun 校准改为使用 `gog` 的 Gmail 账号 `[redacted-email]`，并成功创建 Gmail Draft，收件人为 `[redacted-email]`。
...
## 决策与变更
- 邮件工作流基线变更：以后邮件草稿/发送默认使用 `gog + [redacted-email]`，不使用 Apple Mail；该偏好已写入本地记忆与工具说明。
...
- 错误：邮件草稿最初误走 Apple Mail，违背 Shaun 对 Gmail/gog 的实际偏好。
  - 改进：已把“邮件操作默认走 gog 的 Gmail 账号 `[redacted-email]`”写入 `USER.md` / `TOOLS.md`，后续邮件相关任务先检查该基线。
